...
export function FinancePage() {
  ...
  const kpiInvoiced = useMemo(() => {
    ...
    const received = activeInvoices.reduce((sum, inv) => sum + (inv.amountReceived ?? 0), 0);
                ...
                <div className="md:col-span-2 p-3 rounded-xl border border-[#EEEEEE] bg-[#FAFAFA] flex items-center justify-between">
                  ...
                  <div className="w-1/2 pl-6 flex items-center gap-8">
                    <div className="flex flex-col">
                      <span className="text-xs uppercase tracking-wider font-bold text-[#999999]">Received</span>
                      <span className="text-sm font-bold text-[#0F9D58]">{currencySymbol}{kpiInvoiced.received.toLocaleString()}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs uppercase tracking-wider font-bold text-[#999999]">Due</span>
      ...
      {!collapsed && (
        <table className="w-full">
          <thead className="bg-white border-b border-[#EEEEEE]">
            <tr>
              <th className="px-6 py-2 text-left text-xs text-[#999999] uppercase font-semibold tracking-wider">Requirement</th>
              <th className="px-6 py-2 text-left text-xs text-[#999999] uppercase font-semibold tracking-wider">Type</th>
              <th className="px-6 py-2 text-left text-xs text-[#999999] uppercase font-semibold tracking-wider">Due Date</th>
              <th className="px-6 py-2 text-left text-xs text-[#999999] uppercase font-semibold tracking-wider">Amount</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#EEEEEE]">
            {reqs.map(req => (
              <tr key={req.id} className="hover:bg-[#F9FAFB]">
                ...
                <td className="px-6 py-4">
                  <span className="px-2 py-1 rounded-full bg-[#F7F7F7] text-[#666666] text-xs">
                    {req.type}
                  </span>
                </td>
